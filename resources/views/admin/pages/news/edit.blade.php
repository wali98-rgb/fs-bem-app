  ...
  <div class="row align-items-end">
    <div class="col-lg-8">
      <div class="page-header-title">
        ...
        <div class="d-inline">
          <h4>Mengubah Berita Acara BEM Indonesia Mandiri</h4>
        </div>
      </div>
    </div>
    <div class="col-lg-4">
    ...
    <div class="card-block">
      <form method="POST" enctype="multipart/form-data" action="{{ route('news.update', $news->id) }}">
        ...
        <div class="form-group row">
          ...
          <div class="col-sm-10">
            <input type="text" value="{{ $news->nama }}" name="nama" class="form-control" placeholder="Contoh: LDKM" autofocus>
          </div>
        </div>
        <div class="form-group row">
          <label class="col-sm-2 col-form-label">Tanggal Dilaksanakan</label>
          <div class="col-sm-10">
            <input type="date" value="{{ $news->tanggal }}" name="tanggal" class="form-control" autofocus>
          </div>
        </div>
        <div class="form-group row">
          <label class="col-sm-2 col-form-label">Deskripsi</label>
          <div class="col-sm-10">
            <textarea class="form-control" name="deskripsi" autofocus> {{ $news->deskripsi }} </textarea>
          </div>
        </div>
        <div class="form-group row">
          <label class="col-sm-2 col-form-label">File Upload</label>
          <div class="col-sm-10">
            <input type="file" value="{{ $news->file_berita }}" name="file_berita" class="form-control">
          </div>
        </div>
